...
@JsonCreator @ConstructorProperties("expectedResponseType")
constructor(@JsonProperty("expectedResponseType") expectedResponseType: Class<R>?) :
    AbstractResponseType<Page<R>>(expectedResponseType) {
    override fun matches(responseType: Type): Boolean {
        val unwrapped = ReflectionUtils.unwrapIfType(
            responseType,
            Future::class.java,
            Page::class.java
        )
