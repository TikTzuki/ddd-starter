...
class KStateMachine<STATE : Any, EVENT : Any, SIDE_EFFECT : Any> private constructor(
    ...
    fun with(init: GraphBuilder<STATE, EVENT, SIDE_EFFECT>.() -> Unit): KStateMachine<STATE, EVENT, SIDE_EFFECT> {
    ...
    class Matcher<T : Any, out R : T> private constructor(private val clazz: Class<R>) {
        ...
        fun matches(value: T) = predicates.all {
            it(value)
        }
...
//            inline fun <T : Any, reified R : T> eq(value: R): Matcher<T, R> = any<T, R>().where { this == value }
            inline fun <T : Any, reified R : T> eq(value: R): Matcher<T, R> {
                return any<T, R>().where { this==value }
            }
        }
    }
    ...
    companion object {
        ...
        ): KStateMachine<STATE, EVENT, SIDE_EFFECT> {
        ...
        private fun <STATE : Any, EVENT : Any, SIDE_EFFECT : Any> create(
            ...
            return KStateMachine(GraphBuilder(graph).apply(init).build())
        }
    }
}
